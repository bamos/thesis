...
\hat y = \argmin_y\;\; &\frac{1}{2}||p-y||_2^2  \\
 {\rm s.t.}\;\; & Gy\leq h \\
\end{align*}
Suppose we don't know the polyhedral parameters $\theta=\{G, h\}$
...
\cref{fig:polyhedral-results} shows the results of learning
on two examples.
...
prob = cp.Problem(obj, cons)
layer = cpth.CvxpyLayer(prob, params=[p, A, z], out=[x])
\end{lstlisting}
